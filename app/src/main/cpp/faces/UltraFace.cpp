...
UltraFace::UltraFace(AAssetManager *mgr_, const std::string &mnn_path,
    ...
    // 加载 MNN 模型
    load_model(mgr_, mnn_path);
}


void UltraFace::load_model(AAssetManager *mgr_, std::string mnn_path) {
    // 首先读取文件内容
    ...
    // 关闭
    AAsset_close(asset);
    ultraface_interpreter = std::shared_ptr<MNN::Interpreter>(MNN::Interpreter::createFromBuffer(content.get(), sz));
    MNN::ScheduleConfig config;
    config.numThread = num_thread;
    MNN::BackendConfig backendConfig;
    backendConfig.precision = (MNN::BackendConfig::PrecisionMode) 2;
    config.backendConfig = &backendConfig;
    config.type = MNN_FORWARD_OPENCL;  // 设置使用OpenCL进行后端推理
...
int UltraFace::detect(cv::Mat &raw_image, std::vector<FaceInfo> &face_list) {
    ...
    cv::Mat image;
    cv::resize(raw_image, image, cv::Size(0, 0), in_w / (float)image_w, in_h / (float)image_h);
