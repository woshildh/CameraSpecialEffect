...
Pfld::Pfld(AAssetManager *mgr_, const std::string &mnn_path, int num_thread_){
    // 首先读取文件内容
    AAsset* asset = AAssetManager_open(mgr_, mnn_path.c_str(), AASSET_MODE_UNKNOWN);
    if(asset == NULL) {
        LOGI("[%s] AAssetManager_open failed. Please check %s", "UltraFace", mnn_path.c_str());
        return;
    }
    ...
    AAsset_close(asset);
    //加载模型
    pfld_interpreter = std::shared_ptr<MNN::Interpreter>(MNN::Interpreter::createFromBuffer(content.get(), sz));
    // 配置调度
...
cv::Mat Pfld::Get_Resize_Croped_Img(cv::Mat frame, cv::Point pt1, cv::Point pt2, cv::Point &s_point, cv::Size &croped_wh){
    float cx, cy, halfw;
    ...
    try{
        ...
        if(halfw > 20){
            ...
            cv::resize(croped_img, resize_img, cv::Size(0, 0), (float)INPUT_SIZE / croped_img.cols, (float)INPUT_SIZE / croped_img.rows);
...
void Pfld::Get_Landmark_Points(cv::Mat resize_img, cv::Size croped_wh, cv::Point s_point, vector<LandmarkInfo> &outputs){
    if(resize_img.empty()) {
        return;
    }
